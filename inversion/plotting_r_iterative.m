...
%while rms(diff(:))>=10^(-5) && count<200
    
%while rms(diff(:))>=10^(-4) && count<100
while rms(diff(:))/rms(w_new(:))>=10^(-3) && count<100
...
%save(strcat('/net/graupel/archive1/users/mkohl/inversion/output/r_simulation/rescale',list_end{tt},'/rms5_geostrophic_boundary_w_day',num2str(25*i),'.mat'),'omega','omega_QG','rhs','err_rms','lat');
%save(strcat('/net/graupel/archive1/users/mkohl/inversion/output/r_simulation/rescale',list_end{tt},'/rms4_geostrophic_boundary_w_day',num2str(25*i),'_randn_initial.mat'),'omega','omega_QG','rhs','err_rms','lat');
save(strcat('/net/graupel/archive1/users/mkohl/inversion/output/r_simulation/rescale',list_end{tt},'/rms3_relative_error_geostrophic_boundary_w_day',num2str(25*i),'.mat'),'omega','omega_QG','rhs','err_rms','lat');
