...
function event_analysis_moist(box_min, box_max, p_min, p_max, lat_series_original, lon_series_original, lat_indices, lon_indices, ...
                lons_, days_, precip_, matfilename, input_path, output_path, NETCDF, NETCDF_interval, sigma_tag, sigma_smooth_window)
    ...
    for jj = 1 : length(lat_indices)
        ...
        if ~isempty(lons_{jj})
            ...
            for m = 1 : length(lons_{jj})
                ...
                %disp(['event ', num2str(m), ' of ', num2str(length(lons_{jj}))]);
                ...
                plevels = double(ncread([input_path, 'z_', string_1, '_', num2str(event_year), '.nc'], 'level'));
                ...
                [~, ~, ~, omega, ~, ~, ~] = event_read_Wrapper(computation_y, computation_x, length(level_indices), ...
                        event_year, input_path, event_latspan, event_lonspan, event_day, level_indices, 'historical');
                temp_level = 50000;
                [event_lon, event_lat, tag] = find_center_omega(event_lon, event_lat, ...
                                                omega(:, :, level_indices(plevels == temp_level), 2));
                if ~tag
                ...
                [T, ug, vg, omega, z, omega_b, tag] = event_read_Wrapper(computation_y, computation_x, length(level_indices), ...
                        event_year, input_path, event_latspan, event_lonspan, event_day, level_indices, 'historical');
                if ~tag; continue; end;
                ...
                event_timespan = event_day + [- 0.25, 0.00, 0.25]; % convert to 'days since 1980'
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                ...
                if ~any(reshape(isnan(omega_QG(:, :, :, 2)), 1, []))
                    ...
                    %events_sta{jj}(end).omega_QG_400hPa_full = omega_QG(:, :, level == 40000, 2);
                    events_sta{jj}(end).omega_QG_500hPa_full = omega_QG(y_indices, x_indices, level == 50000, 2);
                    %events_sta{jj}(end).omega_QG_600hPa_full = omega_QG(:, :, level == 60000, 2);
                    ...
                    if NETCDF == true && mod(length(events_sta{jj}), NETCDF_interval) == 0
                        ...
                        nc_filename = [output_path, string_1, '_lat=', num2str(lat_series_original(lat_indices(jj))), ...
