...
%indx = [5,10,17];
indx = [5,round((5+strat_level(tt))/2),strat_level(tt)];
rhs = rhs(:,:,indx,:);
omega = omega(:,:,indx,:);
...
while rms(diff(:))>=1*10^(-4) && count<100
...
asym(count) = Lambda_weighted(w_new(11:26,:,2,1),lat(11:26));
...
save(strcat('/net/graupel/archive1/users/mkohl/inversion/output/abs_simulation/',list{tt},'/rms1em4_geostrophic_boundary_w_day_',num2str(5*(28+i)),'_2layer_alternative_mid.mat'),'omega','omega_QG','r','rhs','lat','err_rms','level');
end
