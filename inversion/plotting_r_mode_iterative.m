...
%while rms(diff(:))>=10^(-8) && count<300
while rms(diff(:))/rms(w_new(:))>=10^(-3) && count<300
...
%save(strcat('/net/graupel/archive1/users/mkohl/inversion/output/r_mode_simulation/rescale',list{tt},'/rms8_geostrophic_boundary_w_day0560h00.mat'),'omega','omega_QG','rhs','err_rms','lat');
save(strcat('/net/graupel/archive1/users/mkohl/inversion/output/r_mode_simulation/rescale',list{tt},'/rms3_relative_error_geostrophic_boundary_w_day0560h00.mat'),'omega','omega_QG','rhs','err_rms','lat');
...
% hc=colorbar;
% title(hc,'Pa/s');
% title('Omega at 500hPA r=0.1')
% xlabel('lon^{\circ}')
% ylabel('lat^{\circ}')
% %savefig('omega_r01_linear.fig')
% %figure(1); saveas(gcf,'Generals_Report/snapshots/omega_linear_r01.png')
% 
% figure(2)
% contour(lon,lat,omega_QG(:,:,11,1)); 
% hc=colorbar;
% title(hc,'Pa/s');
% title('Omega_{QG} at 500hPA r=0.1 ')
% xlabel('lon^{\circ}')
% ylabel('lat^{\circ}')
% %savefig('omegaQG_r01_linear.fig')

% figure(3)
% contour(rhs(:,:,11,1)); colorbar


% %figure(2); saveas(gcf,'Generals_Report/snapshots/omegaQG_linear_r01.png')
% 
% figure(3)
% plot(lon,omega(18,:,11,1)); hold on;
% plot(lon,omega_QG(18,:,11,1))
% legend('omega','QG')
% xlabel('lon')
% ylabel('w(Pa/s)')
% title('w at 500hPA and 40^{\circ} r=0.1')
%savefig('wslice_r01_linear.fig')
%figure(3); saveas(gcf,'Generals_Report/snapshots/wslice_linear_r01.png')
