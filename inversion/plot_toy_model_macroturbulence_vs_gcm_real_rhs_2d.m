...
[lambda_vs_r,lambda_vs_r_1d] = deal(zeros(10,1));
...
rhs_500 = squeeze(rhs(:,:,p500,t));
rhs_500 = rhs_500/max(abs(rhs_500(:)));
rhs_500 = rhs_500-mean(mean(rhs_500));
rhs_500 = -rhs_500; % go from pressure to z coordinates

%[w_final,asymmetry_parameter_1d] = toy_model_nondimensional_generated_spectrum(r,rhs_500(:),dx);

[w_final,asymmetry_parameter] = Omega_Solver_correct(rhs_500,r,Nx,Ny,dx,dy);
...
%lambda_vs_r_1d_new = lambda_vs_r_1d_new + asymmetry_parameter_1d;
...
lambda_vs_r_1d(tt) = lambda_vs_r_1d_new/counter;
...
semilogx(r_gcm,lambda_mode_omega,'b-.'); hold on;
semilogx(r_gcm,lambda_vs_r,'k'); hold on;
%semilogx(r_gcm,lambda_vs_r_1d,'k--'); hold on;
semilogx(r_gcm,lambda_vs_r_spectrum,'r')
xlabel('Reduction factor r')
ylabel('Asymmetry parameter \lambda')
legend('GCM','3-D Inversion','Mode','Theory True RHS','Theory RHS Spectrum','Location','SouthWest'); legend boxoff;
set(gca,'FontSize',12)
