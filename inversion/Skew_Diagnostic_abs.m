...
Skew_Forcing,Skew_Deformation] = deal(zeros(length(surf_temp),1));


for tt = 14:14
tt

[Norm_RHS_i,Norm_Deformation_i,Norm_A1_i,Norm_A1_bb_i,Norm_A1_pp_i,Norm_A1_bp_i,Norm_A1_pb_i,...,
 Norm_A3_i,Norm_zeta_i,Norm_zeta_bar_i,Norm_zeta_prime_i,...,
 Norm_zeta_x_i,Norm_zeta_x_bar_i,Norm_zeta_x_prime_i,...
 Norm_zeta_y_i,Norm_zeta_y_bar_i,Norm_zeta_y_prime_i,...
 Norm_zeta_px_i,Norm_zeta_px_bar_i,Norm_zeta_px_prime_i,...
 Norm_zeta_py_i,Norm_zeta_py_bar_i,Norm_zeta_py_prime_i,...
 Norm_du_dp_i,Norm_dv_dp_i,Norm_du_dp_bar_i,Norm_dv_dp_bar_i,...
 Norm_du_dp_prime_i,Norm_dv_dp_prime_i,...
asym_RHS,asym_A1,asym_A1_bb,asym_A1_pp,asym_A1_pb,asym_A1_bp,...
asym_shear_u,asym_shear_u_bar,asym_shear_u_prime, asym_shear_v,asym_shear_v_bar,asym_shear_v_prime, ...
asym_vor,asym_vor_bar,asym_vor_prime,asym_vor_x,asym_vor_x_bar,asym_vor_x_prime,...,
asym_vor_y,asym_vor_y_bar,asym_vor_y_prime,...
asym_vor_px,asym_vor_px_bar,asym_vor_px_prime,...
asym_vor_py,asym_vor_py_bar,asym_vor_py_prime,...
asym_Forcing,asym_Deformation] = deal(zeros(6,1));
...
[A, B, C, zeta,zeta_p,zeta_x,zeta_y,zeta_px,zeta_py J, Qx, Qy,Forcing, ...
du_dlambda, dv_dlambda, du_dphi, dv_dphi,du_dp,dv_dp ...
...
u_prime = u-mean(u(:,:,:,:),2); v_prime = v-mean(v(:,:,:,:),2);

u_bar = u-u_prime; v_bar = v-v_prime;

du_dp_prime = du_dp-mean(du_dp(:,:,:,:),2); dv_dp_prime = dv_dp - mean(dv_dp(:,:,:,:),2);

du_dp_bar = du_dp - du_dp_prime; dv_dp_bar = dv_dp - dv_dp_prime;
...
[Norm_RHS_p,Norm_Deformation_p,Norm_A1_p,Norm_A3_p,Norm_zeta_p,Norm_zeta_bar_p, ...
Norm_zeta_prime_p,Norm_zeta_x_p,Norm_zeta_x_bar_p, Norm_zeta_x_prime_p,...
...
Norm_A1_bb_p,Norm_A1_pp_p,Norm_A1_bp_p,Norm_A1_pb_p,...
Norm_du_dp_p,Norm_dv_dp_p,Norm_du_dp_bar_p,Norm_du_dp_prime_p,Norm_dv_dp_bar_p,Norm_dv_dp_prime_p] = deal(zeros(length(event_timespan),1));
...
zeta_x_term = zeta_x(:,:,index(tt),t); Norm_zeta_x_p(t) = norm(zeta_x_term(:));
...
zeta_y_term = zeta_y(:,:,index(tt),t); Norm_zeta_y_p(t) = norm(zeta_y_term(:));
...
zeta_px_term = zeta_px(:,:,index(tt),t); Norm_zeta_px_p(t) = norm(zeta_px_term(:));
...
zeta_py_term = zeta_py(:,:,index(tt),t); Norm_zeta_py_p(t) = norm(zeta_py_term(:));
...
Norm_RHS_i(i) = mean(Norm_RHS_p);

Norm_Deformation_i(i) = mean(Norm_Deformation_p);

Norm_A1_i(i) = mean(Norm_A1_p); Norm_A1_bb_i(i) = mean(Norm_A1_bb_p);

Norm_A1_pp_i(i) = mean(Norm_A1_pp_p); Norm_A1_bp_i(i) = mean(Norm_A1_bp_p);

Norm_A1_pb_i(i) = mean(Norm_A1_pb_p);

Norm_A3_i(i) = mean(Norm_A3_p); Norm_zeta_i(i) = mean(Norm_zeta_p);

Norm_zeta_bar_i(i) = mean(Norm_zeta_bar_p); Norm_zeta_prime_i(i) = mean(Norm_zeta_prime_p);

Norm_zeta_x_i(i) = mean(Norm_zeta_x_p);

Norm_zeta_x_bar_i(i) = mean(Norm_zeta_x_bar_p); Norm_zeta_x_prime_i(i) = mean(Norm_zeta_x_prime_p);

Norm_zeta_y_i(i) = mean(Norm_zeta_y_p);

Norm_zeta_y_bar_i(i) = mean(Norm_zeta_y_bar_p); Norm_zeta_y_prime_i(i) = mean(Norm_zeta_y_prime_p);

Norm_zeta_px_i(i) = mean(Norm_zeta_px_p);

Norm_zeta_px_bar_i(i) = mean(Norm_zeta_px_bar_p); Norm_zeta_px_prime_i(i) = mean(Norm_zeta_px_prime_p);

Norm_zeta_py_i(i) = mean(Norm_zeta_py_p);

Norm_zeta_py_bar_i(i) = mean(Norm_zeta_py_bar_p); Norm_zeta_py_prime_i(i) = mean(Norm_zeta_py_prime_p);

Norm_du_dp_i(i) = mean(Norm_du_dp_p); Norm_dv_dp_i(i) = mean(Norm_dv_dp_p); 

Norm_du_dp_bar_i(i) = mean(Norm_du_dp_bar_p); Norm_dv_dp_bar_i(i) = mean(Norm_dv_dp_bar_p); 

Norm_du_dp_prime_i(i) = mean(Norm_du_dp_prime_p); Norm_dv_dp_prime_i(i) = mean(Norm_dv_dp_prime_p); 

asym_RHS(i) = -Skewness(rhs(:,:,index(tt),:));

asym_A1(i) = -Skewness(A1(:,:,index(tt),:));

asym_A1_bb(i) = -Skewness(A1_bb(:,:,index(tt),:));

asym_A1_pp(i) = -Skewness(A1_pp(:,:,index(tt),:));

asym_A1_pb(i) = -Skewness(A1_pb(:,:,index(tt),:));

asym_A1_bp(i) = -Skewness(A1_bp(:,:,index(tt),:));

asym_shear_u(i) = -Skewness(du_dp(:,:,index(tt),:)); asym_shear_u_bar(i) = -Skewness(du_dp_bar(:,:,index(tt),:)); asym_shear_u_prime(i) = -Skewness(du_dp_prime(:,:,index(tt),:));

asym_shear_v(i) = -Skewness(dv_dp(:,:,index(tt),:)); asym_shear_v_bar(i) = -Skewness(dv_dp_bar(:,:,index(tt),:)); asym_shear_v_prime(i) = -Skewness(dv_dp_prime(:,:,index(tt),:));

asym_vor(i) = -Skewness(zeta(:,:,index(tt),:)); asym_vor_bar(i) = -Skewness(zeta_bar(:,:,index(tt),:)); asym_vor_prime(i) = -Skewness(zeta_prime(:,:,index(tt),:));

asym_vor_x(i) = -Skewness(zeta_x(:,:,index(tt),:)); asym_vor_x_bar(i) = -Skewness(zeta_x_bar(:,:,index(tt),:)); asym_vor_x_prime(i) = -Skewness(zeta_x_prime(:,:,index(tt),:));

asym_vor_y(i) = -Skewness(zeta_y(:,:,index(tt),:)); asym_vor_y_bar(i) = -Skewness(zeta_y_bar(:,:,index(tt),:)); asym_vor_y_prime(i) = -Skewness(zeta_y_prime(:,:,index(tt),:));

asym_vor_px(i) = -Skewness(zeta_px(:,:,index(tt),:)); asym_vor_px_bar(i) = -Skewness(zeta_px_bar(:,:,index(tt),:)); asym_vor_px_prime(i) = -Skewness(zeta_px_prime(:,:,index(tt),:));

asym_vor_py(i) = -Skewness(zeta_py(:,:,index(tt),:)); asym_vor_py_bar(i) = -Skewness(zeta_py_bar(:,:,index(tt),:)); asym_vor_py_prime(i) = -Skewness(zeta_py_prime(:,:,index(tt),:));

asym_Forcing(i) =  -Skewness(Forcing(:,:,index(tt),:));

asym_Deformation(i) = -Skewness(Deform(:,:,index(tt),:));


end

% Magnitude
Norm_RHS(tt) = mean(Norm_RHS_i);
Norm_Deformation(tt) = mean(Norm_Deformation_i);
Norm_A1(tt) = mean(Norm_A1_i);
Norm_A1_bb(tt) = mean(Norm_A1_bb_i); Norm_A1_pp(tt) = mean(Norm_A1_pp_i);
Norm_A1_bp(tt) = mean(Norm_A1_bp_i); Norm_A1_pb(tt) = mean(Norm_A1_pb_i);
Norm_A3(tt) = mean(Norm_A3_i);
Norm_zeta(tt) = mean(Norm_zeta_i); Norm_zeta_bar(tt) = mean(Norm_zeta_bar_i); Norm_zeta_prime(tt) = mean(Norm_zeta_prime_i);
Norm_zeta_x(tt) = mean(Norm_zeta_x_i); Norm_zeta_x_bar(tt) = mean(Norm_zeta_x_bar_i); Norm_zeta_x_prime(tt) = mean(Norm_zeta_x_prime_i);
Norm_zeta_y(tt) = mean(Norm_zeta_y_i); Norm_zeta_y_bar(tt) = mean(Norm_zeta_y_bar_i); Norm_zeta_y_prime(tt) = mean(Norm_zeta_y_prime_i);
Norm_du_dp(tt) = mean(Norm_du_dp_i);Norm_dv_dp(tt) = mean(Norm_dv_dp_i); 
Norm_du_dp_bar(tt) = mean(Norm_du_dp_bar_i);Norm_dv_dp_bar(tt) = mean(Norm_dv_dp_bar_i);
Norm_du_dp_prime(tt) = mean(Norm_du_dp_prime_i);Norm_dv_dp_prime(tt) = mean(Norm_dv_dp_prime_i);
Norm_zeta_px(tt) = mean(Norm_zeta_px_i); Norm_zeta_px_bar(tt) = mean(Norm_zeta_px_bar_i); Norm_zeta_px_prime(tt) = mean(Norm_zeta_px_prime_i);
Norm_zeta_py(tt) = mean(Norm_zeta_py_i); Norm_zeta_py_bar(tt) = mean(Norm_zeta_py_bar_i); Norm_zeta_py_prime(tt) = mean(Norm_zeta_py_prime_i);



% Skewness

Skew_RHS(tt) = mean(asym_RHS);

Skew_A1(tt) = mean(asym_A1);

Skew_A1_bb(tt) = mean(asym_A1_bb); Skew_A1_pp(tt) = mean(asym_A1_pp);

Skew_A1_pb(tt) = mean(asym_A1_pb); Skew_A1_bp(tt) = mean(asym_A1_bp);

Skew_shear_u(tt) = mean(asym_shear_u); Skew_shear_u_bar(tt) = mean(asym_shear_u_bar); Skew_shear_u_prime(tt) = mean(asym_shear_u_prime);

Skew_shear_v(tt) = mean(asym_shear_v); Skew_shear_v_bar(tt) = mean(asym_shear_v_bar); Skew_shear_v_prime(tt) = mean(asym_shear_v_prime);

Skew_vor(tt) = mean(asym_vor); Skew_vor_bar(tt) = mean(asym_vor_bar); Skew_vor_prime(tt) = mean(asym_vor_prime);

Skew_vor_x(tt) = mean(asym_vor_x); Skew_vor_x_bar(tt) = mean(asym_vor_x_bar); Skew_vor_x_prime(tt) = mean(asym_vor_x_prime);

Skew_vor_y(tt) = mean(asym_vor_y); Skew_vor_y_bar(tt) = mean(asym_vor_y_bar); Skew_vor_y_prime(tt) = mean(asym_vor_y_prime);

Skew_vor_px(tt) = mean(asym_vor_px); Skew_vor_px_bar(tt) = mean(asym_vor_px_bar); Skew_vor_px_prime(tt) = mean(asym_vor_px_prime);

Skew_vor_py(tt) = mean(asym_vor_py); Skew_vor_py_bar(tt) = mean(asym_vor_py_bar); Skew_vor_py_prime(tt) = mean(asym_vor_py_prime);

Skew_Forcing(tt) = mean(asym_Forcing);

Skew_Deformation(tt) = mean(asym_Deformation);
...
% plot(surf_temp,Skew_vor_y);
% %plot(surf_temp,Skew_Product)
% xlabel('T_{g}')
% ylabel('Skew')
% title('Skew J(\tau,del \phi) Decomposition at 500hPA for abs-turb.')
% legend('Total','du/dp','dv/dp','Zeta','Zeta_x','Zeta_y','location','northwest')
% set(gca, 'TickDir', 'out','Box', 'off','Layer', 'top')
% legend boxoff 
% set(gca,'linewidth',1.5)
...
% ylabel('Skew')
% title('Skew J(\tau,del \phi) Decomposition at 500hPA for abs-turb.')
% legend('A1','A1_{pp}','A1_{pb}','A1_{bp}','location','northwest')
% set(gca, 'TickDir', 'out','Box', 'off','Layer', 'top')
% legend boxoff 
% set(gca,'linewidth',1.5)
...
% xlabel('T_{g}')
% ylabel('Skew')
% title('Skew du/dp Decomposition at 500hPA for abs-turb.')
% legend('Full','Bar','Prime','location','northwest')
% set(gca, 'TickDir', 'out','Box', 'off','Layer', 'top')
% legend boxoff 
% set(gca,'linewidth',1.5)
% savefig('/net/aimsir/archive1/mkohl/vorticity/diagnostic_figures/skew_abs_turbulent_4.fig')
% close all;
% 
% figure
% plot(surf_temp,Skew_shear_v); hold on;
...
% xlabel('T_{g}')
% ylabel('Skew')
% title('Skew dv/dp Decomposition at 500hPA for abs-turb.')
% legend('Full','Bar','Prime','location','northwest')
% set(gca, 'TickDir', 'out','Box', 'off','Layer', 'top')
% legend boxoff 
% set(gca,'linewidth',1.5)
% savefig('/net/aimsir/archive1/mkohl/vorticity/diagnostic_figures/skew_abs_turbulent_5.fig')
% close all;
% 
% figure
% plot(surf_temp,Skew_vor); hold on;
...
% xlabel('T_{g}')
% ylabel('Skew')
% title('Skew Zeta Decomposition at 500hPA for abs-turb.')
% legend('Full','Bar','Prime','location','northwest')
% set(gca, 'TickDir', 'out','Box', 'off','Layer', 'top')
% legend boxoff 
% set(gca,'linewidth',1.5)
% savefig('/net/aimsir/archive1/mkohl/vorticity/diagnostic_figures/skew_abs_turbulent_6.fig')
% close all;
% 
% figure
% plot(surf_temp,Skew_vor_x); hold on;
...
% xlabel('T_{g}')
% ylabel('Skew')
% title('Skew Zeta_x Decomposition at 500hPA for abs-turb.')
% legend('Full','Bar','Prime','location','northwest')
% set(gca, 'TickDir', 'out','Box', 'off','Layer', 'top')
% legend boxoff 
% set(gca,'linewidth',1.5)
% savefig('/net/aimsir/archive1/mkohl/vorticity/diagnostic_figures/skew_abs_turbulent_7.fig')
% close all;
% 
% figure
...
% xlabel('T_{g}')
% ylabel('Skew')
% title('Skew Zeta_y Decomposition at 500hPA for abs-turb.')
% legend('Full','Bar','Prime','location','northwest')
% set(gca, 'TickDir', 'out','Box', 'off','Layer', 'top')
% legend boxoff 
% set(gca,'linewidth',1.5)
% savefig('/net/aimsir/archive1/mkohl/vorticity/diagnostic_figures/skew_abs_turbulent_8.fig')
% close all;
% 
% figure
...
% xlabel('T_{g}')
% ylabel('Skew')
% title('Skew J(\tau,del \tau) Decomposition at 500hPA for abs-turb.')
% legend('Forcing','du/dp','dv/dp','zeta_{px}','zeta_{py}','location','northwest')
% set(gca, 'TickDir', 'out','Box', 'off','Layer', 'top')
% legend boxoff 
% set(gca,'linewidth',1.5)
% savefig('/net/aimsir/archive1/mkohl/vorticity/diagnostic_figures/skew_abs_turbulent_9.fig')
% close all;
% 
% figure
% plot(surf_temp,Skew_vor_px); hold on;
...
% xlabel('T_{g}')
% ylabel('Skew')
% title('Skew zeta_{px} Decomposition at 500hPA for abs-turb.')
% legend('Full','Bar','Prime','location','northwest')
% set(gca, 'TickDir', 'out','Box', 'off','Layer', 'top')
% legend boxoff 
% set(gca,'linewidth',1.5)
% savefig('/net/aimsir/archive1/mkohl/vorticity/diagnostic_figures/skew_abs_turbulent_10.fig')
% close all;
% 
% figure
% plot(surf_temp,Skew_vor_py); hold on;
...
% xlabel('T_{g}')
% ylabel('Skew')
% title('Skew zeta_{py} Decomposition at 500hPA for abs-turb.')
% legend('Full','Bar','Prime','location','northwest')
% set(gca, 'TickDir', 'out','Box', 'off','Layer', 'top')
% legend boxoff 
% set(gca,'linewidth',1.5)
% savefig('/net/aimsir/archive1/mkohl/vorticity/diagnostic_figures/skew_abs_turbulent_11.fig')
% close all;
% 
figure(4)
%set(gcf,'DefaultAxesFontSize', 14)
...
legend('Total','A1_{pp}','A1_{pb}','A1_{bp}','location','northeast')
set(gca, 'TickDir', 'out','Box', 'off','Layer', 'top')
legend boxoff 
set(gca,'linewidth',1.5)
...
% xlabel('T_{g}')
% ylabel('Norm')
% title('Zeta_x Decomposition at 500hPA for abs-turb.')
% legend('Bar','Prime','location','northwest')
% set(gca, 'TickDir', 'out','Box', 'off','Layer', 'top')
% legend boxoff 
% set(gca,'linewidth',1.5)
% savefig('/net/aimsir/archive1/mkohl/vorticity/diagnostic_figures/mag_abs_turbulent_6.fig')
% close all
% 
% figure(10)
% %set(gcf,'DefaultAxesFontSize', 12)
...
% xlabel('T_{g}')
% ylabel('Norm')
% title('Zeta_y Decomposition at 500hPA for abs-turb.')
% legend('Bar','Prime','location','northwest')
% set(gca, 'TickDir', 'out','Box', 'off','Layer', 'top')
% legend boxoff 
% set(gca,'linewidth',1.5)
% savefig('/net/aimsir/archive1/mkohl/vorticity/diagnostic_figures/mag_abs_turbulent_7.fig')
% close all
% 
% figure(11)
% %set(gcf,'DefaultAxesFontSize', 12)
...
% xlabel('T_{g}')
% ylabel('Norm')
% title('Zeta_{px} Decomposition at 500hPA for abs-turb.')
% legend('Bar','Prime','location','northwest')
% set(gca, 'TickDir', 'out','Box', 'off','Layer', 'top')
% legend boxoff 
% set(gca,'linewidth',1.5)
% savefig('/net/aimsir/archive1/mkohl/vorticity/diagnostic_figures/mag_abs_turbulent_8.fig')
% close all;
% 
% figure(12)
% %set(gcf,'DefaultAxesFontSize', 12)
...
% xlabel('T_{g}')
% ylabel('Norm')
% title('Zeta_{py} Decomposition at 500hPA for abs-turb.')
% legend('Bar','Prime','location','northwest')
% set(gca, 'TickDir', 'out','Box', 'off','Layer', 'top')
% legend boxoff 
% set(gca,'linewidth',1.5)
% savefig('/net/aimsir/archive1/mkohl/vorticity/diagnostic_figures/mag_abs_turbulent_9.fig')
% close all;
% 
% 
