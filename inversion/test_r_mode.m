...
list1 = {'Total_rhs_smooth0_iter20_day0560h00.mat'};
...
path_temp = strcat('/disk7/mkohl/interpolation/output_r_mode/data_',list{tt},'/temp_day0560h00.nc');
path_omega = strcat('/disk7/mkohl/interpolation/output_r_mode/data_',list{tt},'/omega_day0560h00.nc');
...
strat_level(tt) = round(mean(mean(max_level)),0);
        
% Calculate the Asymmetry Factor vs. r 

omega = omega - mean(omega(:,:,:,:),2);

Asymmetry(tt) = mean(Lambda_weighted(omega(:,:,1:strat_level(tt),:),lat));
