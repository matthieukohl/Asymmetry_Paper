...
[spec_av_r_mode,spec_mid_r_mode,spec_500_r_mode] = deal(zeros(10,256));
...
[k_av_new,spec_av_new] = Spectrum2_weighted(omega(:,:,plow:trop_new,t),lat,lon);
[k_mid_new,spec_mid_new] = Spectrum2_weighted(omega(:,:,pmid,t),lat,lon);
[k_500_new,spec_500_new] = Spectrum2_weighted(omega(:,:,p500,t),lat,lon);

% [k_av_new,spec_av_new] = Spectrum2_weighted(v(:,:,plow:trop_new,t),lat,lon);
% [k_mid_new,spec_mid_new] = Spectrum2_weighted(v(:,:,pmid,t),lat,lon);
% [k_500_new,spec_500_new] = Spectrum2_weighted(v(:,:,p500,t),lat,lon);


%S_500_new = mean(mean(sigma_accu(:,:,p500,t),1),2);

weight = repmat(cosd(lat),1,length(lon));
S_500_new = sum(sum(weight.*sigma_accu(:,:,p500,t),1),2)/(sum(sum(weight,1),2));
...
% now calculate the spectrum for the modes

list = {'0.01','0.02','0.05','0.1','0.2','0.3','0.4','0.6','0.8','1.0'};

[k_av_r_mode,k_mid_r_mode,k_500_r_mode] = deal(zeros(length(R_factor),1));
...
[k_av_new,spec_av_new] = Spectrum2_weighted(omega(12,:,plow:trop_new,t),lat(12),lon);
[k_mid_new,spec_mid_new] = Spectrum2_weighted(omega(12,:,pmid,t),lat(12),lon);
[k_500_new,spec_500_new] = Spectrum2_weighted(omega(12,:,p500,t),lat(12),lon);
...
spec_av_r_mode(tt,:) = spec_av/counter;
spec_mid_r_mode(tt,:) = spec_mid/counter;
spec_500_r_mode(tt,:) = spec_500/counter;

k_av_r_mode(tt) = k_av/counter;
k_mid_r_mode(tt) = k_mid/counter;
k_500_r_mode(tt) = k_500/counter;
...
semilogx(k(1:N/2),spec_500_r_mode(1,1:N/2)/max(spec_500_r_mode(1,1:N/2)));
...
figure(2)
semilogx(R_factor,k_500_r.*Ld,'b'); hold on
semilogx(R_factor,k_500_r_mode.*Ld,'r');
