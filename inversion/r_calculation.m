% Calculates the average r for the turbulent phase of the abs simulations.
...
r_day(i-4) = mean(mean(mean(mean(r(:,:,1:strat_level_turbulent(tt),:),1),2),3),4);
r_day_500(i-4) = mean(mean(mean(mean(r(:,:,6,:),1),2),3),4);

%lambda_day = toy_model_randn(mean(sigma,2),mean(r,4),lat,lon,level,phi,dphi,lambda,dlambda,strat_level_turbulent(tt));

%r_day(i) = mean(mean(mean(mean(r(:,:,6,:),1),2),3),4); %500hPa

end

r_abs(tt) = mean(r_day);
r_abs_500(tt) = mean(r_day_500);
