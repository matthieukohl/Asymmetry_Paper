...
%while rms(diff(:))>=1*10^(-5) && count<100
while rms(diff(:))/rms(w_new(:))>=1*10^(-3) && count<100
...
% format long
error(count) = rms(diff(:))/rms(w_new(:));
...
%save(strcat('/net/graupel/archive1/users/mkohl/inversion/output/abs_simulation/',list{tt},'/rms1em4_geostrophic_boundary_w_day',num2str(5*(28+i)),'.mat'),'omega','omega_QG','r','rhs','lat','err_rms');
%save(strcat('/net/graupel/archive1/users/mkohl/inversion/output/abs_simulation/',list{tt},'/rms1em5_geostrophic_boundary_w_day',num2str(5*(28+i)),'.mat'),'omega','omega_QG','r','rhs','lat','err_rms');
save(strcat('/net/graupel/archive1/users/mkohl/inversion/output/abs_simulation/',list{tt},'/rms1em3_relative_error_geostrophic_boundary_w_day',num2str(5*(28+i)),'.mat'),'omega','omega_QG','r','rhs','lat','err_rms');
end
...
contour(lon,lat,omega(:,:,11,1)); 
hc=colorbar;
title(hc,'Pa/s');
title('Omega')
xlabel('lon^{\circ}')
ylabel('lat^{\circ}')
% %savefig('omega_r01_linear.fig')
%saveas(gcf,'Plots_Generals/final/omega_nonlinear_abs','epsc')
% 
figure('pos', [10, 10, 1000, 300])
contour(lon,lat,omega_QG(:,:,11,1)); 
hc=colorbar;
title(hc,'Pa/s');
title('Omega_{QG}')
xlabel('lon^{\circ}')
ylabel('lat^{\circ}')
% %savefig('omegaQG_r01_linear.fig')
%saveas(gcf,'Plots_Generals/final/omegaQG_nonlinear_abs','epsc')
...
title('w-profile')
xlim([lon(1) lon(end)])
%saveas(gcf,'Plots_Generals/final/wslice_nonlinear_abs','epsc')
% 
% figure('pos', [10, 10, 1000, 300])
% contour(lon,lat,r(:,:,11,1)); 
% hc=colorbar;
% title('r')
% xlabel('lon^{\circ}')
% ylabel('lat^{\circ}')
% caxis([0 0.9])
% saveas(gcf,'Plots_Generals/final/r_nonlinear_abs','epsc')

% figure(5)
% plot(lon,omega(11,:,11,1)); hold on;
% plot(lon,r(11,:,11,1)); hold on;
% plot(lon,10^(16)*rhs(11,:,11,1)); hold on;