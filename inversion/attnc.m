...
%
% modified by pog to remove opendap and to use native netcdf commands
%
%--------------------------------------------------------------------
% DESCRIPTION:
%
...
%  [cdfid, rcode] = mexnc('ncopen', full_name, 'nowrite');
  cdfid = netcdf.open(full_name, 'nowrite'); % pog
  ...
  if strcmp(var_name, 'global')
    ...
    %end
    [num_dims, nvars, ngatts, recdim] = netcdf.inq(cdfid); % pog
    if isempty(att_name)
      if ngatts > 0
	for i = 0:ngatts-1
	  %[attnam, rcode] = mexnc('attname', cdfid, 'global', i);
          varid_global = netcdf.getConstant('NC_GlOBAL'); % pog
          attnam = netcdf.inqAttName(cdfid, varid_global, i); % pog
	  %[attype, attlen, rcode] = mexnc('ncattinq', cdfid, 'global', attnam);
          [attype, attlen] = netcdf.inqAtt(cdfid, varid_global, attnam); % pog
	  %[values, rcode] = mexnc('ncattget', cdfid, 'global', attnam);
          values = netcdf.getAtt(cdfid, varid_global, attnam); % pog
	  att_val{i+1} = values;
    ...
    %end
    varid = netcdf.inqVarID(cdfid, var_name); % pog

    %[varname, datatype, num_dims, dim, natts, rcode] = ...
%	mexnc('VARINQ', cdfid, varid);
    [varname, datatype, dim, natts] = netcdf.inqVar(cdfid, varid); % pog
    num_dims = length(dim); % pog
	...
	for i = 0:natts-1
	  %[attnam, rcode] = mexnc('attname', cdfid, varid, i);
          attnam = netcdf.inqAttName(cdfid, varid, i); % pog
	  %[attype, attlen, rcode] = mexnc('ncattinq', cdfid, varid, attnam);
          [attype, attlen] = netcdf.inqAtt(cdfid, varid, attnam); % pog
	  %[values, rcode] = mexnc('ncattget', cdfid, varid, attnam);
          values = netcdf.getAtt(cdfid, varid, attnam); % pog
	  att_val{i+1} = values;
	  att_name_list{i+1} = attnam;
	end
      else
