...
omega_QG(:, :, :, t) = SIP_diagnostic(R, f0, length(lon), length(phi), length(level), phi, lambda, level, ...
                                                            sigma(:, t), dphi, dlambda, rhs(:,:,:,t),  omega(:,:,:,t));
...
omega_QG(:, :, :, t) = SIP_sigma(R, f0, length(lon), length(phi), length(level), phi, lambda, level, ...
                                   sigma_new_smoothed(:, :, :, t), dphi, dlambda, rhs(:, :, :, t),  omega(:,:,:,t));   
...
save(strcat('output/rescale_0.1/sigma/relax_mean/smooth_gauss/Total_rhs_smooth0_sigma_smooth2.1_iter10_day',num2str(25*i),'.mat'),'omega','omega_QG','rhs','sigma','sigma_new','sigma_new_smoothed');
