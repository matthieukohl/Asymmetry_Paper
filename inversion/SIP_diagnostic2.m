function omega = SIP_diagnostic1(r, f, Ni, Nj, Nk, phi, lambda, level, sigma, dphi, dlambda, rhs,omega_bound, omega_b)
    ...
    resmax = 1e-14; %1e-14 25th July
    alpha = 0.93; %convergence parameter; 11th June originally alpha=0.93;
    %%%%%%%%%%%%%%%%%%%%%%%
    ...
    for j = 1 : Nj
        for i = 1 : Ni
            ijk = Lk(1) + Li(i) + j;
            %T(ijk) = 0;
            T(ijk) = omega_bound(j, i, 1);
            ijk = Lk(Nk) + Li(i) + j;
            T(ijk) = 0;
            %T(ijk) = omega_bound(j, i, Nk);
        end
    end
    ...
    for k = 2 : Nkm
        for j = 2 : Njm
            for i = 2 : Nim
                ...
                AE(ijk) = AW(ijk);
                %AB(ijk) = dzr2;
                AB(ijk) = 2 * f(j)^2 / (dp1 * (dp1 + dp2));    
                %AT(ijk) = dzr2; 
                AT(ijk) = 2 * f(j)^2 / (dp2 * (dp1 + dp2));
                %Q(ijk) = 0.0;
