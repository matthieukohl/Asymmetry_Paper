...
[lambda_vs_r,lambda_vs_r_max,lambda_vs_r_mean] = deal(zeros(10,1));
...
[k_500_new,spec_rhs_gcm] = Spectrum2(rhs(:,:,p500,t),lat,lon);
...
[rhs_gen] = generate_spectrum(spec_rhs_gcm,L);
[w_final,asymmetry_parameter] = toy_model_nondimensional_generated_spectrum(r_gcm(tt),real(rhs_gen),dx);
...
lambda_vs_lat(jj) = lambda_pholder/draws;
...
legend('GCM','3-D Inversion','Theory True RHS (Mean)','Theory True RHS (Peaks)','Theory True RHS (Max)','Theory RHS Spectrum','Location','SouthWest'); legend boxoff;
set(gca,'FontSize',12)