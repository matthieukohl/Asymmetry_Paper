...
path = ['/net/graupel/archive1/users/mkohl/inversion/output/r_mode_simulation/rescale',list{tt},'/rms6_geostrophic_boundary_',low,'_day0560h00_two_layer_alternative.mat'];
...
pholder = Lambda_weighted(omega(:,:,2,:),lat);
...
pholder = Lambda_weighted(omega_QG(:,:,2,:),lat);
...
path = [path,'/rms4_geostrophic_boundary_',low,'_day',num2str(25*(8+jj)),'_2layer_alternative.mat'];
...
lambda_omega = lambda_omega + Lambda_weighted(omega(:,:,2,:),lat);
...
lambda_omega_QG = lambda_omega_QG + Lambda_weighted(omega_QG(:,:,2,:),lat);
