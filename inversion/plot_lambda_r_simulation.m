...
%path = ['/net/graupel/archive1/users/mkohl/inversion/output/r_mode_simulation/rescale',list{tt},'/rms3_relative_error_geostrophic_boundary_',low,'_day0560h00.mat'];
...
%pholder = Lambda_weighted(omega(:,:,11,:),lat);
...
%pholder = Lambda_weighted(omega_QG(:,:,11,:),lat);
...
%path = [path,'/rms3_relative_error_geostrophic_boundary_',low,'_day',num2str(25*(8+jj)),'.mat'];
...
%lambda_omega = lambda_omega + Lambda_weighted(omega(:,:,11,:),lat);
...
%lambda_omega_QG = lambda_omega_QG + Lambda_weighted(omega_QG(:,:,11,:),lat);
...
% save(['figures_paper/lambda_r_',low,'_',averaging,'_rms1em3_relative_error.mat'],'lambda_turb_omega','lambda_turb_omega_QG','lambda_turb_omega_QG_rhs',...
%     'lambda_mode_omega','lambda_mode_omega_QG','lambda_mode_omega_QG_rhs')
...
semilogx(r,lambda_turb_omega_QG,'b:','linewidth',1.2); hold on;
semilogx(r,lambda_mode_omega,'r','linewidth',1.2); hold on;
semilogx(r,lambda_mode_omega_QG,'r:','linewidth',1.2); hold on;
xlabel('r');
%ylabel('\lambda');
set(gca,'FontSize',14);
title('r+RHS')
...
%save('test_full.mat','lambda_mode_omega','lambda_mode_omega_QG','lambda_turb_omega','lambda_turb_omega_QG');
save('test500.mat','lambda_mode_omega','lambda_mode_omega_QG','lambda_turb_omega','lambda_turb_omega_QG');


%saveas(gcf,['figures_paper/inversions_r_',low],'epsc')