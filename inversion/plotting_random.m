...
for tt = 1:length(r)
tt
[asymmetry] = deal(zeros(length(runs),1));
i=10;

path_temp = strcat('/disk7/mkohl/interpolation/output1/data_',list{tt},'/temp_day0',num2str(25 * i, '%.3d'),'h00.nc');
...
%sigma = nanmean(nanmean(sigma(1:strat_level(tt),:),1),2);

%sigma = repmat(sigma,length(level),1);

sigma = nanmean(sigma,2);
...
asymmetry_r(tt) = nanmean(asymmetry);
...
figure(2)
semilogx(r,asymmetry_r); hold on;
xlabel('Updraught Stability Reduction Factor r');
ylabel('Asymmetry Parameter {\lambda}');
title('Asymmetry Parameter Gauss-Forcing')
ylim([0.5 1])
saveas(gcf,'figures_paper/lambda_white_noise_rhs','epsc')

%lambda_theory_constantN2 = []
