function [gregorian_time, serial_time, gregorian_base, serial_base, ...
	  sizem, serial_time_jd, serial_base_jd, ...
          use_leap_year, use_equal_months] = ...
    timenc(file, time_var, bl_corner, tr_corner, calendar)
%
% Modified by pog to remove opendap and use native netcdf
% Modified by pog to return information on leap years and length of months
% (note just returns use_leap_year=1 for proleptic_gregorian)
%
...
%  end
  cdfid = netcdf.open(full_name, 'NC_NOWRITE'); % pog
  ...
  %end
  varid = netcdf.inqVarID(cdfid, time_var); % pog
...
%    end
    ...
    [varnam, vartypv, vdims, nvatts] = netcdf.inqVar(cdfid, varid); % pog
    nvdims = length(vdims); % pog
...
%      end
     [varnam, sizem(i)] = netcdf.inqDim(cdfid, vdims(i)); % pog
    end
    ...
    if any(sizem == 0)
      serial_rel_int = []; % Presumably time is an unlimited dimension of length 0.
...
%      end
     serial_rel_int = netcdf.getVar(cdfid, varid, st, sizem); % pog
    end
  else
%    [varnam, vartypv, nvdims, vdims, nvatts, rcode] = ...
%	mexnc('ncvarinq', cdfid, varid);
...
%	error(['** ERROR ** ncdiminq: rcode = ' num2str(rcode)])
%      end
      [name, sizem(i)] = netcdf.inqDim(cdfid, vdims(i)); % pog
      if tr_corner(i) < 0
	tr_corner(i) = sizem(i);
      end
    end
...
%	error(['** ERROR ** ncvarget: rcode = ' num2str(rcode)])
%      end
      serial_rel_int = netcdf.getVar(cdfid, varid, bl_corner-1, tr_corner-bl_corner+1); % pog
    end
  end

% pog: necessary to recast from int to double when using native netcdf
  serial_rel = cast(serial_rel_int, 'double'); % pog
  ...
  %end
  base_str = netcdf.getAtt(cdfid, varid, 'units'); % pog
...
% pog mod: return information leap year and length of months
...
switch lower(calendar)
 ...
 case {'all_leap', '366_day'}
  error('all_leap or 366 day not handled by modified timenc')
 case '360_day'
  use_leap_year = 0;
  use_equal_months = 1;
 otherwise
