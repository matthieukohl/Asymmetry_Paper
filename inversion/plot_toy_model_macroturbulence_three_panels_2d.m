...
for ii = 1:length(r)
    for jj = 1:length(k)
     ...
     % symmetric rhs    
     [~,lambda(ii,jj)] = toy_model_2d(r(ii),k(jj));
...
lgd = legend({'k=1.2','k=1.5','k=1.8','k=6.6'},'Location','NorthEast');
...
w_profile = zeros(300,300,length(r_sample));

for ii = 1:length(r_sample)
[w_profile(:,:,ii),~] = toy_model_2d(r_sample(ii),k(k1p6));
end

x = linspace(0,2*pi/k(k1p6),300);

w_profile = squeeze(w_profile(77,:,:));
...
saveas(gcf,'figures_paper/toy_model_asymmetry_and_w_profile_three_panels_2d','epsc');
