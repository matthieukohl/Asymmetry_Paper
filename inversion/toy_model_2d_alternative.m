function [w_final,asymmetry_parameter,asymmetry_parameter_2] = toy_model_2d_alternative(r,k)
...
% check the asymmetry parameter against the estimate
% lambda = 1 - a_u/(a_u+a_d);

index_up = w_final>0;
index_down = w_final<0;

asymmetry_parameter_2 = 1 - sum(index_up(:))/(sum(index_up(:))+sum(index_down(:)));


% field = w_final;
% L = 2*pi*p/k;
% %field_average = mean(mean(field,1),2);
% field_average = 1/L*(trapz(x,trapz(x,field,1),2));
% field_prime = field-field_average;
% 
% field_up = max(field(:,:,:,:),0);
% %field_up_average = mean(mean(field_up(:,:,:,:),1),2);
% field_up_average = 1/L*(trapz(x,trapz(x,field_up,1),2));
...
% % test something
% lap_w = Lap(N,Ny,dx,dy)*w_old;
% 
% ku = sqrt(0.5*mean(abs(lap_w(w_old>0)))/mean(abs(w_old(w_old>0))));
% 
% kd = sqrt(0.5*mean(abs(lap_w(w_old<0)))/mean(abs(w_old(w_old<0))));
% 
% k = 2*ku*kd/(ku+kd)

% ratio = lap_w./w_old;
% 
% ku = sqrt(0.5*mean(abs(ratio(w_old>0))));
% kd = sqrt(0.5*mean(abs(ratio(w_old<0))));
% k = 2*ku*kd/(ku+kd)
