...
%save(strcat('/net/graupel/archive1/users/mkohl/inversion/output/abs_mode_simulation/',list{tt},'/rms15_geostrophic_w_rhs_day0560h00.mat'),'omega','omega_QG','r','rhs','lat');
save(strcat('/net/graupel/archive1/users/mkohl/inversion/output/abs_mode_simulation/',list{tt},'/rms15_geostrophic_w_rhs_day0560h00_old_div_form.mat'),'omega','omega_QG','r','rhs','lat');
...
hc=colorbar;
title(hc,'Pa/s');
title('Omega at 500hPA r=0.4')
xlabel('lon^{\circ}')
ylabel('lat^{\circ}')
% %savefig('omega_r01_linear.fig')
%saveas(gcf,'Plots_Generals/snapshots/omega_nonlinear_r04','epsc')
% 
figure('pos', [10, 10, 1000, 300])
contour(lon,lat,omega_QG(:,:,11,1)); 
hc=colorbar;
title(hc,'Pa/s');
title('Omega_{QG} at 500hPA r=0.4 ')
xlabel('lon^{\circ}')
ylabel('lat^{\circ}')
% %savefig('omegaQG_r01_linear.fig')
%saveas(gcf,'Plots_Generals/snapshots/omegaQG_nonlinear_r04','epsc')
...
figure(3)
plot(lon,omega(15,:,11,1)); hold on;
plot(lon,omega_QG(15,:,11,1))
legend('omega','QG')
xlabel('lon')
ylabel('w(Pa/s)')
title('w at 500hPA and 40 lat, r=0.4')
xlim([lon(1) lon(end)])
%saveas(gcf,'Plots_Generals/snapshots/wslice_nonlinear_r04','epsc')
