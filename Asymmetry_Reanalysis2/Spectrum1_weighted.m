function [Wavenumber_avg,spec] = Spectrum1_weighted(omega,lat,lon)
...
k_centroid = sum(k(indx:end).*W_power(indx:end)')/sum(W_power(indx:end));

Wavenumber(jj) = k_centroid;

end

Wavenumber_avg = sum(cosd(lat).*Wavenumber)/sum(cosd(lat));

weight = repmat(cosd(lat),1,length(lon));

spec = squeeze(sum(weight.*Power,1)./sum(weight,1));
