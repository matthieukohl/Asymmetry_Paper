...
[~,p200] = min(abs(level-200));

lambda_ncep2_NH_x_av = squeeze(mean(mean(lambda_ncep2_x(lat70:lat30,1:p200,:),1),2));
...
lambda_erai_NH_x_av = squeeze(mean(mean(lambda_erai_NH_x(lat70:lat30,p200:end,:),1),2));
...
lambda_era5_NH_x_av = squeeze(mean(mean(lambda_era5_NH_x(lat70:lat30,p200:end,:),1),2));
...
% w = cosd(lat(lat70:lat30));
% lambda_era5_NH_x_weighted = squeeze(sum(w/sum(w).*lambda_era5_NH_x(lat70:lat30,p500,:),1));
% lambda_era5_NH_xt_weighted = squeeze(sum(w/sum(w).*lambda_era5_NH_xt(lat70:lat30,p500,:),1));
...
load('data_theory/theory_NH_jul26th.mat'); 
load('data_theory/theory_SH_jul26th.mat');
...
lambda_NH_theory = lambda_NH_theory_500;
lambda_SH_theory = lambda_SH_theory_500;
...
lambda_NH_modal_theory = lambda_NH_modal_theory_500;
lambda_SH_modal_theory = lambda_SH_modal_theory_500;
