...
[k_NH_max_rhs,k_NH_centroid_rhs,k_SH_max_rhs,k_SH_centroid_rhs] = deal(zeros(12,1));
load('lon.mat');
...
LD_NH_500 = squeeze(Ld_NH(16,:));
LD_SH_500 = squeeze(Ld_SH(16,:));
...
k_NH_centroid_rhs(ii) = sum(k_NH(1:N/2).*spec_rhs_winter(1:N/2))./sum(spec_rhs_winter(1:N/2));
k_SH_centroid_rhs(ii) = sum(k_SH(1:N/2).*spec_rhs_winter(1:N/2))./sum(spec_rhs_winter(1:N/2));
...
% calculate with k centroid from rhs-spectrum
...
saveas(gcf,'/net/aimsir/archive1/mkohl/inversion/figures_paper/lambda_seasonal_reanalysis_vs_theory_new_all_reanalyses_k_rhs_centroid','epsc');
...
plot(k_NH_centroid_rhs,'k','Linewidth',1.5); hold on;
plot(k_SH_centroid_rhs,'k--','Linewidth',1.5)
