...
[spec_NH_mid,spec_NH_500,spec_NH_av] = deal(zeros(144,12));
else
...
[spec_SH_mid,spec_SH_500,spec_SH_av] = deal(zeros(144,12));
end
...
% file was stored in graupel because aimsir was full
file = ['/net/graupel/archive1/users/mkohl/reanalysis/asymmetry_project/ncep2/air.', num2str(year),'.nc'];

% file stored in aimsir
file_omega = ['/net/aimsir/archive1/mkohl/era_interim/Asymmetry_Project/ncep2/omega.', num2str(year),'.nc'];
...
omega_in = ncread(file_omega,'omega',start,count);
...
lat30 = latm70; % this is needed because we need to start from smaller to larger index
lat70 = latm30;
end

% Define f-factor
f = 2*Omega*sind(lat(lat30:lat70)); % Coriolis parameter
beta = 2*Omega/R*cosd(lat(lat30:lat70)); % beta-factor


% find the dominant Wavenumber

[k_av_new,spec_av_new] = Spectrum1_weighted(omega(lat30:lat70,:,plow:trop_new),lat(lat30:lat70),lon);
[k_mid_new,spec_mid_new] = Spectrum1_weighted(omega(lat30:lat70,:,pmid),lat(lat30:lat70),lon);
[k_500_new,spec_500_new] = Spectrum1_weighted(omega(lat30:lat70,:,p500),lat(lat30:lat70),lon);
...
weight = repmat(cosd(lat(lat30:lat70)),1,length(lon),length(level));

[S_mid_new,S_500_new,S_av_new] = Static_Stability_weighted(sigma_accu(lat30:lat70,:,:),level,trop_new,weight);
...
[L_mid_new,L_500_new,L_av_new] = Deformation_Radius_weighted(sigma_accu(lat30:lat70,:,:),f,level,lat,trop_new,weight);
...
[r_mid_new,r_500_new,r_av_new] = reduction_factor_weighted(r(lat30:lat70,:,:),level,trop_new,weight);
...
[r_mid_cond_new,r_500_cond_new,r_av_cond_new] = reduction_factor_conditioned_on_ascent(r(lat30:lat70,:,:),omega(lat30:lat70,:,:),level,trop_new);
...
%   
 ...
 save('data_theory/theory_ncep2_NH_july12th_weighted.mat', ...
      'lambda_NH_theory_mid','lambda_NH_theory_500','lambda_NH_theory_av',...
      'lambda_NH_modal_theory_500',...
  ...
  'trop_NH','S_500_NH','S_mid_NH','S_av_NH', 'lat', 'level')
   ...
   save('data_theory/theory_ncep2_SH_july12th_weighted.mat', ...
      'lambda_SH_theory_mid','lambda_SH_theory_500','lambda_SH_theory_av',...
  'lambda_SH_modal_theory_500',...   
