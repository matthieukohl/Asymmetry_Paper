% Calculate the reduction factor and k value
% for era5 coarse grained
...
[spec_NH_mid,spec_NH_500,spec_NH_av] = deal(zeros(240,12));
else
...
[spec_SH_mid,spec_SH_500,spec_SH_av] = deal(zeros(240,12));
end
...
lat_original = ncread(file,'latitude'); lat_original = double(lat_original); lat_original = flip(lat_original);
lon_original = ncread(file,'longitude');lon_original = double(lon_original);
level = ncread(file,'level'); level = double(level)*100; level = flip(level);

% Define f-factor
f = 2*Omega*sind(lat_original); % Coriolis parameter
beta = 2*Omega/R*cosd(lat_original); % beta-factor
...
start = [1, 1, 1, t];
count = [length(lon_original), length(lat_original), length(level), 1];

start_sf = [1, 1, t];
count_sf = [length(lon_original), length(lat_original),1];
...
T = rearrange_era(T_in,lat_original,lon_original,level);
omega = rearrange_era(omega_in,lat_original,lon_original,level);

clear('omega_in','T_in');

% % coarse grain omega and T: from 0.25 res to 1.25 res
% 
% omega = movmean(movmean(omega,6,1),6,2); % averaging
% 
% omega = omega(1:5:end,1:5:end,:); % downsampling
% 
% T = movmean(movmean(T,6,1),6,2); % averaging
% 
% T = T(1:5:end,1:5:end,:); % downsampling
% 
% lat = lat_original(1:5:end);
% lon = lon_original(1:5:end);

% coarse grain omega and T: from 0.25 res to 1.5 res
...
% % coarse grain omega and T: from 0.25 res to 1.75 res
% 
% omega = movmean(movmean(omega,8,1),8,2); % averaging
% 
% omega = omega(1:7:end,1:7:end,:); % downsampling
% 
% T = movmean(movmean(T,8,1),8,2); % averaging
% 
% T = T(1:7:end,1:7:end,:); % downsampling
% 
% lat = lat_original(1:7:end);
% lon = lon_original(1:7:end);
...
if tf_hemisphere ==1
 ...
 save('data_theory/theory_era5_NH_coarse_grained_res_1p5.mat', ...
      'lambda_NH_theory_mid','lambda_NH_theory_500','lambda_NH_theory_av',...
      'lambda_NH_modal_theory_500',...
  ...
  'trop_NH','S_500_NH','S_mid_NH','S_av_NH', 'lat', 'level')
   ...
   save('data_theory/theory_era5_SH_coarse_grained_res_1p5.mat', ...
      'lambda_SH_theory_mid','lambda_SH_theory_500','lambda_SH_theory_av',...
  'lambda_SH_modal_theory_500',...   
