...
%  disp('The base time has no string for the hour, assume hour = min = sec = 0')
  ...
  if isempty(str)
    ...
    [str, remainder] = strtok(remainder);
% pog modification to deal with gfdl 2.1 malformed time header
    if ~isempty(findstr('W',str))|~isempty(findstr('Factor',str))
     disp('pog modification to parsetnc: garbage found in time attribute')
    end
    if isempty(str)|~isempty(findstr('W',str)) |~isempty(findstr('Factor',str))
...
% pog modification to deal with gfdl 2.1 malformed time header
if ~isempty(remainder)&isempty(findstr('W',str))&isempty(findstr('Factor',str))
